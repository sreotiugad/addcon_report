...
def get_n_data(d_from, d_to, logs=None):
    ...
    for acc in NAVER_ACCOUNTS:
        ...
        acc_start_idx = len(rows)  # ✅ 이 계정 rows 시작 인덱스
        ...
        for camp in camps:
            ...
            for grp in adgroups:
                if grp.get("nccAdgroupId"):
                    tasks.append((camp, grp))

        logs.append(f"[NAVER] 전체 라이브 그룹 수: {len(tasks)}")
        if not tasks:
            continue
        ...
        with ThreadPoolExecutor(max_workers=20) as executor:
            futures = {executor.submit(fetch_grp, task): task for task in tasks}
            done = 0
        ...
        try:
            ...
            if conv_rows:
                ...
                # ✅ 이 계정 rows만 대상으로 머지 (acc_start_idx ~ acc_end_idx)
                matched = 0
                for i in range(acc_start_idx, acc_end_idx):
                    grp_id = rows[i].get("그룹ID", "")
                    dt     = rows[i].get("날짜", "")
                    dev    = rows[i].get("기기", "")
                    match  = df_conv_all[
        ...
        try:
            ...
            for b in stream:
                for r in b.results:
                    ...
                    count += 1
            logs.append(f"✅ [Google cost] customer_id={cust_id} rows={count}")
        except Exception as e:
            logs.append(f"❌ [Google cost] customer_id={cust_id} 오류: {e}")

        # ✅ 쿼리2 - 가입 전환 (별도 try/except)
        try:
